...
def screen_momentum(tickers, benchmark="SPY"):
    results = []
    bench_hist = get_hist(benchmark, "1y")
    bench_ret_63  = (bench_hist["Close"].iloc[-1] / bench_hist["Close"].iloc[-63]  - 1) * 100
    bench_ret_252 = (bench_hist["Close"].iloc[-1] / bench_hist["Close"].iloc[-252] - 1) * 100

    for sym in tickers:
        try:
            h = get_hist(sym, "1y")
            if h.empty or len(h) < 63:
                continue
            cur = h["Close"].iloc[-1]
            mom = momentum_score(h)
            pct_high, pct_low, h52, l52 = week52_proximity(h)
            rel_63  = mom.get(63,  np.nan) - bench_ret_63
            rel_252 = mom.get(252, np.nan) - bench_ret_252
            results.append({
                "Ticker":      sym,
                "Price":       round(cur, 2),
                "1M %":        round(mom.get(21,  np.nan), 1),
                "3M %":        round(mom.get(63,  np.nan), 1),
                "6M %":        round(mom.get(126, np.nan), 1),
                "12M %":       round(mom.get(252, np.nan), 1),
                "vs SPY 3M":   round(rel_63,  1),
                "vs SPY 12M":  round(rel_252, 1),
    ...
    if df.empty:
        return df
    # composite momentum score (equal weight 1M 3M 6M 12M)
...
def screen_sector_momentum():
    ...
    for name, sym in SECTOR_ETFS.items():
        try:
            ...
            cur = h["Close"].iloc[-1]
            mom = momentum_score(h)
            pct_high, _, h52, _ = week52_proximity(h)
            results.append({
                "Sector": name,
                "ETF": sym,
                "Price": round(cur, 2),
...
try:
    ...
    hist_6m = hist_1y.tail(126)

    if hist_1y.empty:
        st.error(f"No price data for {ticker_input}")
        st.stop()
    ...
    if run_screen:
        ...
        if df_screen.empty:
            ...
            for i, sym in enumerate(top10):
                try:
                    h = get_hist(sym, "1y")
                    norm_close = h["Close"] / h["Close"].iloc[0] * 100
            ...
            try:
                rel_hist = get_hist(relative_to, "1y")
    ...
    if run_sector:
        ...
        if df_sect.empty:
            ...
            for i, (_, row) in enumerate(df_sect.iterrows()):
                try:
                    h = get_hist(row["ETF"], "1y")
                    nc = h["Close"] / h["Close"].iloc[0] * 100
